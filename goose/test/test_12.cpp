...
int
main() {
    std::cout << "hello, world to libPEG.h!" << std::endl;

    const auto parse_tree_factory = PEG::ParseTree::PlainParseTreeFactory::instance<PEG::ParseTree::PlainParseTreeFactory>();

    auto parser = PEG::Parser(make_grammar(parse_tree_factory));
    const auto parseResult = parser.parse("3");
    parseResult->output();
